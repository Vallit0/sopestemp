  ...
  const handleFormSubmit = async (e) => {
    ...
    if (uploadPicture) {
      if (uploadOption === "upload" && imageFile) {
        ...
        setregifsterStatus("Por favor seleccione una imagen o tome una foto");
        setVariantStatus("danger");
        return;
      }
    }

    // console.log(fullName, username, password, imageFile);

    // console.log(formData)

    try {
      const response = await fetch(`${basePythonUrl}/auth/register`, {
        method: "POST",
        body: formData,
      });
      if (response.ok) {
  ...
  return (
    <>
      <Row>
        ...
        <Col md={6} style={{ background: "#101010", textAlign: "center" }}>
          <img
            src="cat2.jpg"
            alt="cat picture"
            style={{ height: "100vh", width: "76%", margin: "auto" }}
          />
        </Col>
      </Row>
    </>
